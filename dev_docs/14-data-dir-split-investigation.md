...
### 10.4 唯一需要专门设计的边界：`setup` 的冷启动 ✅ 已定（采纳 A）

`setup` 在**任何 hook 跑过之前**就要运行 → 那时指针还不存在 → `getDataDir()` 会抛错（10.2 的③）。**已定采纳方案 A**：

- **✅ A（采纳）**：让"首次有效配置"不依赖写盘 —— API 凭证已走环境变量（见 `status.md`），其余用代码内置默认；把"需要落盘的配置"（`mode` / `use` 改的 `config.json` / `spaces.json`）的写入时机推迟到"用户至少发过一条消息、指针已就绪"之后。**`setup` 退化为"校验 env + 打印下一步指引"，不写数据目录**，因此 `setup` 不再负责初始化 `spaces.json`（改由首条消息后的 hook / `use` 落盘）。这同时顺带解决了 B6（`setup.md` 的 bash heredoc 跨平台问题）—— 不写盘就没有那段 heredoc。
- ~~B：`setup` 末尾主动触发 hook 自检催生指针~~ —— 不采纳（复杂、依赖未确认能力）。

### 10.5 定稿后的实施顺序

1. **数据清零** ✅ 已确认。注意：**测试数据在另一台设备上，本机两个 data 目录里的内容均非真实/重要数据**，故本机无需备份、无迁移动作；正规安装后真实测试设备上从空 `data.db` 自然重来即可。本机若有残留目录，直接删除无虞。
