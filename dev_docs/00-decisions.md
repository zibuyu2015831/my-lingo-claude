...
`additionalContext` 方式在极端情况下（原始输入非常长且语义复杂）Claude 仍可能同时参考原始输入与优化版本。这是 Claude Code hook 机制的结构性上限（无法删除原始输入），通过优化指令措辞可以缓解，无法根治。用户在极端情况下可使用 `!raw` 前缀跳过优化，手动发送英文输入。
