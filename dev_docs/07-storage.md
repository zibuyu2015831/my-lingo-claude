...
├── analysis.lock      # 分析进程互斥锁（v0.6，含 PID，5 分钟新鲜窗口）
├── debug.log          # MY_LINGO_DEBUG=1 时的调试日志（写前脱敏 + 1MB 轮转）
├── learning/<space>/  # /my-lingo:lesson 生成的课程 markdown（lessons-YYYY-MM-DD.md）
└── data.db            # SQLite 单库（WAL 模式，伴随运行时的 data.db-wal / data.db-shm）
```

> 另：env-blind slash 命令的定位指针固定写在 `~/.claude/plugins/data/my-lingo/install.json`（由 hook 维护，见 dev_docs/14），不在本数据目录内。
...
> 完整默认值见 `config.mjs::DEFAULT_CONFIG`。`display_mode` 默认 `full`（非 compact）。凭证字段（`api_key`/`api_base_url`/`model_fast`/`model_deep`）属 `CREDENTIAL_FIELDS`，读写 config.json 时强制过滤，**不会出现在此文件中**。

### 4.2 API 凭证：plugin userConfig 优先，环境变量兜底

API 凭证不得写入任何文件。两种配置来源，**优先级 plugin userConfig > 环境变量**：

| config 字段 | 优先：plugin userConfig（注入为环境变量）| 兜底：用户 export | 用途 | 是否必填 |
|---------|------|------|------|---------|
| `api_key` | `CLAUDE_PLUGIN_OPTION_API_KEY` | `MY_LINGO_API_KEY` | API 认证密钥 | 必填 |
| `api_base_url` | `CLAUDE_PLUGIN_OPTION_API_BASE_URL` | `MY_LINGO_API_BASE_URL` | API 端点，如 `https://api.openai.com/v1` | 必填 |
| `model_fast` | `CLAUDE_PLUGIN_OPTION_MODEL_FAST` | `MY_LINGO_MODEL_FAST` | 同步优化使用的快速模型 | 必填 |
| `model_deep` | `CLAUDE_PLUGIN_OPTION_MODEL_DEEP` | `MY_LINGO_MODEL_DEEP` | 异步分析使用的深度模型 | 选填（默认等于 model_fast） |

凭证在 `loadConfig()` 中作为最高优先级（Layer 0）通过 `credValue()` 解析：对每个字段先取 `CLAUDE_PLUGIN_OPTION_*`（Claude Code 把 plugin.json userConfig 值注入为该环境变量），为空时回退到用户自行 export 的 `MY_LINGO_*`。`writeConfig()` 会在写入前自动过滤凭证字段，防止意外落地。

**两种配置方式：**
- 插件安装界面填写 userConfig（推荐，优先生效；`sensitive` 字段存入系统 keychain）
- 或手动 export 环境变量作为兜底：macOS / Linux 写入 `~/.zshrc` / `~/.bashrc`；Windows 走系统属性 → 高级 → 环境变量
...
**path-hash 推导（`stop.mjs`，含 F8 修复）：**
```js
const hash = cwd.replace(/[^a-zA-Z0-9]/g, '-')
// 示例：/data/zibuyu/my_lingo_claude → -data-zibuyu-my-lingo-claude
```
所有非字母数字字符都替换为 `-`（对齐 Claude Code 真实规则）。早期只替换 `/`、`_`，含 `.`/空格/`@` 的路径会算错目录（见 `15` F8）。
