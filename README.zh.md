...
| 命令 | 作用 |
|------|------|
| `/my-lingo:setup` | 首次配置：API 地址、密钥、模型、语言 |
| `/my-lingo:status` | 查看当前配置、活跃语言空间、今日统计 |
...
```
/my-lingo:setlang japanese    ← 切换到日语空间
/my-lingo:status              ← 确认当前活跃空间
```
...
├── config.json                   # API 密钥和全局配置（权限 0600）
├── spaces.json                   # 语言空间配置
├── circuit.json                  # 熔断器状态（自动管理）
├── turns/
│   └── YYYY-MM-DD.jsonl          # 每日 Prompt 记录
└── learning/
    └── english/
        ├── corrections-YYYY-MM.jsonl   # 语法纠错记录
        ├── items-YYYY-MM.jsonl         # 词汇与句式（SRS）
        └── lessons-YYYY-MM-DD.md      # 生成的课程
```
