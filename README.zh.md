...
母语非英文的开发者在使用 Claude Code 时，往往同时面对两个难题：

1. **Prompt 质量** — 同样的需求，用中文（或日文、韩文等）描述，往往不如一段结构清晰的英文 Prompt 更能让 Claude 准确理解你的意图。
2. **语言学习** — 你最该掌握的技术英语，恰恰藏在真实的编程对话里，而不是教科书中。

My Lingo 把这两件事合二为一。

每当你提交一条 Prompt，它会先通过外部 LLM API 把内容改写成结构清晰的英文，再透明地交给 Claude —— 整个过程你几乎察觉不到。

你的原始消息永远不会被拦下：万一 API 不可用，Prompt 就原样发出。而每次会话结束后，My Lingo 还会自动分析这些改写记录，从中提取纠错与词汇，把你每天的真实交互沉淀成一个专属的语言学习库。
...
这两个默认值是按**慢速推理模型**（如 `gemini-2.5-pro`、o 系列）的需要设定的：这类模型会把大量 token 预算花在看不见的推理过程上，预算给小了，输出的 JSON 就会被截断，分析也就"悄无声息"地一无所获。几点提醒：

- `deep_timeout_seconds` **必须小于** `hooks/hooks.json` 中 `SessionEnd` 的 `timeout`（默认 `60`）。否则分析还没来得及写入，就被 hook 提前终止，而相关的 turn 已被标记为"已分析"，这部分学习数据便永久丢失。
- 若希望会话结束几乎即时完成，可把 deep 模型换成**快速的非推理模型**（如 `deepseek-chat`），再把这两个值调小。
- Prompt 优化（**fast** 模型）链路会自动计算自己的 token 预算，不受这两个设置影响。
